...
export default function PasswordChecker() {
    ...
    const strokeDashoffset = strokeDasharray - (strokeDasharray * strength.score) / 100;

    // Dynamic color logic for HEX (SVG) and Tailwind classes
    const getColor = (score) => {
        if (score === 0 || !password) return { hex: 'transparent', bg: 'bg-slate-700', text: 'text-slate-400' };
        if (score < 25) return { hex: '#ef4444', bg: 'bg-red-500', text: 'text-red-500' }; // red
        if (score < 50) return { hex: '#f97316', bg: 'bg-orange-500', text: 'text-orange-500' }; // orange
        if (score < 75) return { hex: '#eab308', bg: 'bg-yellow-500', text: 'text-yellow-500' }; // yellow
    ...
    return (
        <div className="bg-[#111827] border border-slate-700 rounded-2xl p-8 shadow-2xl">
            <h2 className="text-2xl font-bold text-white mb-2">Password Strength Checker</h2>
            <p className="text-sm text-slate-400 mb-6">Enter a password to analyze its strength across multiple security parameters</p>
            ...
            <div className="flex flex-col items-center justify-center mb-8">
                <div className="relative w-40 h-40 flex items-center justify-center">
                    ...
                    <div className="absolute flex flex-col items-center justify-center">
                        <span className={`text-4xl font-bold ${password ? colors.text : 'text-slate-500'}`}>{strength.score}</span>
                        <span className={`text-xs ${password ? colors.text : 'text-slate-500'}`}>{strength.label || "Awaiting Input"}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#0f172a] border border-slate-700 p-4 rounded-xl">
                    ...
                    <div className="text-emerald-400 text-xl font-bold">{details.length}</div>
                </div>
                <div className="bg-[#0f172a] border border-slate-700 p-4 rounded-xl">
                    <div className="text-slate-400 text-sm mb-1">Uppercase</div>
                    <div className="text-emerald-400 text-xl font-bold">{details.uppercase}</div>
                </div>
                <div className="bg-[#0f172a] border border-slate-700 p-4 rounded-xl">
                    <div className="text-slate-400 text-sm mb-1">Lowercase</div>
                    <div className="text-emerald-400 text-xl font-bold">{details.lowercase}</div>
                </div>
                <div className="bg-[#0f172a] border border-slate-700 p-4 rounded-xl">
                    <div className="text-slate-400 text-sm mb-1">Numbers</div>
                    <div className="text-emerald-400 text-xl font-bold">{details.numbers}</div>
                </div>
            </div>
        </div>
    );
